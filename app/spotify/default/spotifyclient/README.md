# Go API client for spotifyclient
...
For using other server than the one defined on index 0 set context value `spotifyclient.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `spotifyclient.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `spotifyclient.ContextOperationServerIndices` and `spotifyclient.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), spotifyclient.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, spotifyclient.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
